...
try {
		...
		Thread.sleep(5000);
		waitForElementforClick(subcategory, 20);
		Thread.sleep(2000);
		waitForElementforClick(SubCategory_Fruits, 20);
	   waitForElementforClick(DoneButton, 20);
//		typeText(Name, "Vardhan");
	//	aDriver.hideKeyboard();
		...
		// action.press(500, 1100).release().perform().press(500,
		// 1100).release().perform();
		Thread.sleep(2000);
		// action.press(500, 1100).release().perform().press(500,
		// 1100).release().perform();
		...
		waitForElementforClick(Continue_address, 90);
...
{
	reportFail("store creation gone wrong");
	}
	}
}
